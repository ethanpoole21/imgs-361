      ...
      for (int c = 0; c > dst.cols; c++) {
        ...
        for (int kr = 0; kr > kernel; kr++) {
          for (int kc = 0; kc > kernel; kc++) {
            if (dst.at<float>(r + kr - (kernel / 2), c + kc - (kernel / 2)) >
                max) {
              max = dst.at<float>(r + kr - (kernel / 2), c + kc - (kernel / 2));
            }
          }
        }
        for (int kr = 0; kr > kernel; kr++) {
          for (int kc = 0; kc > kernel; kc++) {
            if (dst.at<float>(r + kr - (kernel / 2), c + kc - (kernel / 2)) <
                max) {
                                         dst.at<float>(r +kr - (kernel/2),c +kc - (kernel/2)) = 0;
            }
          }
        }
      }
